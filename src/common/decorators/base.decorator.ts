...
import {
  ADDED_IN_PREFIX,
  DEPRECATED_IN_PREFIX,
  LIFECYCLE_EXTENSION,
} from '@/common/constants';
import { ImmichWorker, JobName, MetadataKey, QueueName } from '@/common/enums';
import { EmitEvent } from '@/infrastructure/event';
import { setUnion } from '@/utils/set';
...
function chunks<T>(
  ...
  if (collection instanceof Set) {
    ...
    const chunksArray: Array<Array<T>> = [];
  ...
  return (
    ...
    descriptor.value = async function (
      ...
      if (
        ...
        const value = originalMethod(...arguments_);
        return value;
      }

      const list: Array<unknown> = Array.isArray(argument)
        ? (argument as Array<unknown>)
        : Array.from(argument as Set<unknown>);

      const merged: unknown = await Promise.all(
        chunks(list, chunkSize).map(async (chunk) => {
        ...
        options.mergeFn ? options.mergeFn(results) : results,
      );
      return merged;
    };
  };
...
export function ChunkedArray(options?: {
  paramIndex?: number;
}): MethodDecorator {
  return Chunked({
    ...options,
    mergeFn: (results: unknown[]) => results.flat(1),
  });
}
...
export function ChunkedSet(options?: { paramIndex?: number }): MethodDecorator {
  return Chunked({
    ...options,
    mergeFn: (results: unknown[]) => setUnion(...(results as Set<unknown>[])),
  });
}
...
export const GenerateSql = (...options: GenerateSqlQueries[]) =>
  SetMetadata(GENERATE_SQL_KEY, options);
...
export const EndpointLifecycle = ({
  ...
  const decorators: MethodDecorator[] = [
    ApiExtension(LIFECYCLE_EXTENSION, { addedAt, deprecatedAt }),
  ];
  if (deprecatedAt) {
    decorators.push(
      ApiTags('Deprecated'),
...
export const PropertyLifecycle = ({
  addedAt,
  deprecatedAt,
}: LifecycleMetadata) => {
