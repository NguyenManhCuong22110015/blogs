...
import { PropertyLifecycle } from '@/common/decorators';
import {
  CLIPConfig,
  DuplicateDetectionConfig,
  FacialRecognitionConfig,
} from '@/common/dtos/model-config';
import {
  AudioCodec,
  CQMode,
  Colorspace,
...
const isLibraryScanEnabled = (config: SystemConfigLibraryScanDto) =>
  config.enabled;
const isOAuthEnabled = (config: SystemConfigOAuthDto) => config.enabled;
const isOAuthOverrideEnabled = (config: SystemConfigOAuthDto) =>
  config.mobileOverrideEnabled;
const isEmailNotificationEnabled = (config: SystemConfigSmtpDto) =>
  config.enabled;
const isDatabaseBackupEnabled = (config: DatabaseBackupConfig) =>
  config.enabled;
...
class SystemConfigJobDto
  implements Record<ConcurrentQueueName, JobSettingsDto>
{
...
class SystemConfigMachineLearningDto {
  ...
  @ArrayMinSize(1)
  @Transform(({ obj, value }: { obj: { url?: string }; value: string[] }) =>
    obj && typeof obj === 'object' && obj.url ? [obj.url] : value,
  )
  @ValidateIf((dto: SystemConfigMachineLearningDto) => dto.enabled)
  @ApiProperty({
    type: 'array',
