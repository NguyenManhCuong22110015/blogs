...
import { citiesFile, excludePaths, IWorker } from '@/common/constants';
import { Telemetry } from '@/common/decorators';
import { EnvDto } from '@/common/dtos/env.dto';
import {
  DatabaseExtension,
  ImmichEnvironment,
  ImmichHeader,
...
import { DatabaseConnectionParams, VectorExtension } from '@/common/types';
...
const getEnv = (): EnvData => {
  ...
  if (errors.length > 0) {
    ...
    for (const error of errors) {
      messages.push(
        `  - ${error.property}=${error.value} (${Object.values(error.constraints || {}).join(', ')})`,
      );
    }
  ...
  if (redisUrl && redisUrl.startsWith('ioredis://')) {
    try {
      redisConfig = JSON.parse(
        Buffer.from(redisUrl.slice(10), 'base64').toString(),
      );
    } catch (error) {
  ...
  return {
    ...
    resourcePaths: {
      ...
      geodata: {
        ...
        naturalEarthCountriesPath: join(
          folders.geodata,
