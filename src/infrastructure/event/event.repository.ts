...
import { ImmichWorker, MetadataKey, QueueName } from '@/common/enums';
import { ConfigRepository } from '@/infrastructure/config';
import { LoggingRepository } from '@/infrastructure/logging';
import { handlePromiseError } from '@/utils/misc';
import { JobItem, JobSource } from '@/common/types';
...
type EventMap = {
  ...
  AssetMetadataExtracted: [
    { assetId: string; userId: string; source?: JobSource },
  ];
...
export interface ClientEventMap {
  ...
  on_server_version: [any];
  on_config_update: [];
  on_new_release: [any];
  on_notification: [any];
  on_session_delete: [string];

  AssetUploadReadyV1: [{ asset: any; exif: any }];
}
...
export type AuthFn = (client: Socket) => Promise<any>;
  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit
{
  ...
  setAuthFn(fn: (client: Socket) => Promise<any>) {
    this.authFn = fn;
  }
