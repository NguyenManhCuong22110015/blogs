...
import { AuthApiKey } from '@/common/types';
import { User } from '@prisma/client';

// TODO: Define these types or import from appropriate modules
type AuthSession = any;
type AuthSharedLink = any;
type AuthUser = User;
type UserAdmin = User;
import { ImmichCookie, UserMetadataKey } from '@/common/enums';
import { UserMetadataItem } from '@/common/types';
import { Optional, PinCode, toEmail } from '@/utils/validation';
...
export function mapLoginResponse(
  ...
  const onboardingMetadata = entity.metadata.find(
    (item): item is UserMetadataItem<UserMetadataKey.Onboarding> =>
      item.key === UserMetadataKey.Onboarding,
  )?.value;
