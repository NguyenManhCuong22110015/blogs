...
import { SystemConfig, defaults } from '@/common/config/system.conf';
import { SystemConfigDto } from '@/common/dtos/system-config.dto';
import { DatabaseLock, SystemMetadataKey } from '@/common/enums';
...
import { getKeysDeep, unsetDeep } from '@/utils/misc';

export type SystemConfigValidator = (
  config: SystemConfig,
...
export const getConfig = async (
  ...
  if (!withCache || !config) {
    ...
    await asyncLock.acquire(
      DatabaseLock[DatabaseLock.GetSystemConfig],
      async () => {
...
export const updateConfig = async (
  ...
  for (const property of getKeysDeep(defaults)) {
    ...
    const isEqual =
      newValue === defaultValue || _.isEqual(newValue, defaultValue);
...
const buildConfig = async (repos: RepoDeps) => {
  ...
  if (
    !config.ffmpeg.acceptedVideoCodecs.includes(config.ffmpeg.targetVideoCodec)
  ) {
